...
var a = new Float32Array([
	...
	constructor(e, t, n) {
		...
		let r = e.createVertexArray(), o = e.createBuffer();
		if (!r || !o) throw Error("Failed to create glow geometry");
		this.vao = r, this.buffer = o, e.bindVertexArray(r), e.bindBuffer(e.ARRAY_BUFFER, o), e.bufferData(e.ARRAY_BUFFER, a, e.STATIC_DRAW);
		let s = e.getAttribLocation(this.program, "aPosition");
		e.enableVertexAttribArray(s), e.vertexAttribPointer(s, 2, e.FLOAT, !1, 0, 0), e.bindVertexArray(null), this.uTime = e.getUniformLocation(this.program, "uTime");
	}
	draw(e) {
		let t = this.gl;
...
function p(e) {
	let t = Math.max(e.width, 1), n = Math.max(e.height, 1), r = t * 2.2, i = n * .8, a = t * .6, o = n * .8, c = r + i + a + o, l = Math.random() * c;
	if (l < r) {
		let n = Math.random(), r = e.left + n * t;
		return Math.random() < .24 && (r = Math.random() < .5 ? e.left + s(0, 90) : e.right - s(0, 90)), {
			x: r,
			y: e.top,
			normalX: 0,
			normalY: 1,
...
function b(e, t, n) {
	return e === "burning" ? w(t, n) : C(t, n);
}
function x(e, t) {
	let n = d(e, "burning"), r = s(1, 2.6);
	return {
		x: n.x + s(-14, 14),
		y: n.y + s(-4, 14),
		z: -2,
		vx: n.normalX * s(4, 18) + s(-16, 16),
		vy: s(18, 54),
		maxLife: r,
...
function S(e, t, n, r, i) {
	let a, o, s;
	if (i === "burning") if (r) a = 1, o = c(.82, .14, n), s = c(.16, .02, n);
	else if (n < .18) {
		let e = n / .18;
		a = 1, o = c(.98, .68, e), s = c(.56, .08, e);
	} else if (n < .58) {
		let e = (n - .18) / .4;
		a = 1, o = c(.68, .16, e), s = c(.08, .01, e);
	} else {
		let e = (n - .58) / .42;
		a = c(1, .38, e), o = c(.16, .02, e), s = 0;
	}
	else if (r) a = 1, o = c(.65, .1, n), s = .015;
	else if (n < .22) {
		let e = n / .22;
		a = 1, o = c(.95, .58, e), s = c(.45, .03, e);
	} else if (n < .62) {
		let e = (n - .22) / .4;
		a = 1, o = c(.58, .13, e), s = c(.03, .005, e);
	} else {
		let e = (n - .62) / .38;
		a = c(1, .32, e), o = c(.13, .01, e), s = 0;
	}
	e[t * 3] = a, e[t * 3 + 1] = o, e[t * 3 + 2] = s;
}
function C(e, t) {
	let n = d(e, "border"), r = Math.random() < .12, i = {
		x: n.x + s(-2, 2),
		y: n.y + s(-2, 2),
		z: s(-1, 1),
		vx: 0,
	...
	return r ? (i.vx = n.normalX * s(20, 80) + s(-35, 35), i.vy = s(55, 160), i.maxLife = s(.7, 1.5), i.baseSize = s(2, 6)) : (i.vx = n.normalX * s(8, 38) + s(-16, 16), i.vy = s(25, 105), n.side === "top" && (i.vy += s(25, 90)), i.maxLife = s(.35, 1.05), i.baseSize = s(13, 32)), (n.side === "left" || n.side === "right") && (i.vy *= s(.7, 1.15)), i.life = t ? Math.random() * i.maxLife : i.maxLife, i;
}
function w(e, t) {
	let n = d(e, "burning"), r = Math.random() < .17, i = {
		x: n.x + s(-3, 3),
		y: n.y + s(-3, 3),
		z: s(-1, 1),
		vx: 0,
	...
	return r ? (i.vx = n.normalX * s(40, 120) + s(-55, 55), i.vy = s(90, 220), i.baseSize = s(3, 7), i.maxLife = s(.45, 1.2)) : (i.vx = n.normalX * s(14, 46) + s(-26, 26), i.vy = s(70, 180), i.baseSize = s(14, 38), i.maxLife = s(.35, 1.15), n.side === "top" && (i.vy += s(40, 110), i.baseSize += s(4, 14)), (n.side === "left" || n.side === "right") && (i.vx += n.normalX * s(8, 28)), Math.random() < .12 && (i.baseSize *= 1.45, i.vy *= 1.18)), i.life = t ? Math.random() * i.maxLife : i.maxLife, i;
}
//#endregion
...
var T = class {
	...
	constructor(t = {}) {
		...
		this.gl = r, this.fire = new m(r, this.options.particleCount, v, h), n === "burning" ? this.fire.setStyle(1.15, .18, .56) : this.fire.setStyle(1.05, .12, .52), this.smoke = this.options.smokeCount > 0 ? new m(r, this.options.smokeCount, v, y) : null, this.glow = this.options.glow ? new o(r, _, g) : null, this.options.skipGreeting || e("agent-aura");
	}
	setTarget(e) {
	...
	updateBounds() {
		...
		this.bounds = u(t, e, this.options.padding);
	}
	seedParticles(e) {
		this.fireData = [];
		for (let t = 0; t < this.options.particleCount; t++) this.writeFire(t, b(this.options.preset, this.bounds, e));
		if (this.smokeData = [], this.smoke) for (let t = 0; t < this.options.smokeCount; t++) this.writeSmoke(t, x(this.bounds, e));
	}
	writeFire(e, t) {
	...
	render(e, t) {
		...
		for (let n = 0; n < this.options.particleCount; n++) {
			...
			if (i.life -= t, i.life <= 0) {
				this.writeFire(n, b(r, this.bounds, !1));
				continue;
			}
			...
			r === "burning" ? (o = a < .1 ? a / .1 : 1 - (a - .1) / .9, o = Math.max(o, 0) ** 1.2, i.spark ? (this.fire.alphas[n] = o * 1.32, this.fire.sizes[n] = i.baseSize * (1 - a * .55)) : (this.fire.alphas[n] = o * 1, this.fire.sizes[n] = i.baseSize * (.65 + a * 1.55))) : (o = Math.max(o, 0) ** 1.25, i.spark ? (this.fire.alphas[n] = o * 1.22, this.fire.sizes[n] = i.baseSize * (1 - a * .55)) : (this.fire.alphas[n] = o * .86, this.fire.sizes[n] = i.baseSize * (.58 + a * 1.4))), S(this.fire.colors, n, a, i.spark, r);
		}
		if (this.smoke) {
			for (let n = 0; n < this.options.smokeCount; n++) {
				let r = this.smokeData[n];
				if (r.life -= t, r.life <= 0) {
					this.writeSmoke(n, x(this.bounds, !1));
					continue;
				}
...
//#endregion
export { E as i, O as n, D as r, k as t };
